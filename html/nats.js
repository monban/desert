...
class Request1 {
    ...
    constructor(mux1, opts1 = {
        ...
        extend1(this, opts1);
        this.timer = timeout1(opts1.timeout);
    }
    resolver(err, msg) {
        if (this.timer) {
            this.timer.cancel();
...
class JsMsgImpl {
    ...
    next(subj, ro) {
        ...
        if (ro) {
            ...
            payload = DataBuffer1.concat(NXT, SPACE, data);
        }
        const opts1 = subj ? {
            reply: subj
        } : undefined;
        this.msg.respond(payload, opts1);
    }
    term() {
        this.doAck(TERM);
    }
...
function isRequestError(msg1) {
    if (msg1 && msg1.headers) {
        const headers2 = msg1.headers;
...
class SubscriptionImpl1 extends QueuedIteratorImpl {
    ...
    cleanupFn;
    closed;
    constructor(protocol1, subject1, opts2 = {
    }){
        super();
        extend1(this, opts2);
        ...
        this.draining = false;
        this.noIterator = typeof opts2.callback === "function";
...
function assert(cond, msg4 = "Assertion failed.") {
    if (!cond) {
        throw new AssertionError(msg4);
    }
}
...
class Connect1 {
    ...
    version;
    headers;
    constructor(transport1, opts3, nonce){
        this.protocol = 1;
        this.version = transport1.version;
        this.lang = transport1.lang;
        this.echo = opts3.noEcho ? false : undefined;
        this.verbose = opts3.verbose;
        this.pedantic = opts3.pedantic;
        this.tls_required = opts3.tls ? true : undefined;
        this.name = opts3.name;
        const creds = (opts3 && opts3.authenticator ? opts3.authenticator(nonce) : {
        }) || {
        };
        extend1(this, creds);
    }
...
class ProtocolHandler1 {
    ...
    push(e) {
        switch(e.kind){
            case Kind1.MSG:
                {
                    const { msg: msg4 , data: data2  } = e;
                    this.processMsg(msg4, data2);
                    break;
                }
            case Kind1.OK: break;
            case Kind1.ERR:
    ...
    initMux() {
        const mux2 = this.subscriptions.getMux();
        if (!mux2) {
...
function defaultJsOptions(opts4) {
    ...
    if (opts4.domain) {
        opts4.apiPrefix = `$JS.${opts4.domain}.API`;
        delete opts4.domain;
    }
    return extend1({
...
class ListerImpl {
    ...
    jsm;
    filter;
    constructor(subject2, filter, jsm1){
        if (!subject2) {
    ...
    advisories() {
        ...
        this.nc.subscribe(`$JS.EVENT.ADVISORY.>`, {
            callback: (err, msg4)=>{
                ...
                try {
                    const d = this.parseJsResponse(msg4);
        ...
        const sub = this.nc.subscribe(inbox, {
            ...
            callback: (err, msg4)=>{
                ...
                if (err !== null) {
                    ...
                    if (isNatsError1(err) && err.code === ErrorCode1.JetStream404NoMessages) {
                        qi.stop();
                    } else {
                        qi.stop(err);
                    }
...
class JetStreamSubscriptionImpl extends TypedSubscription1 {
    constructor(js2, subject4, opts11){
        super(js2.nc, subject4, opts11);
    }
    set info(info) {
        this.sub.info = info;
    }
...
class EncodedBucket1 {
    bucket;
    codec;
    constructor(bucket1, codec1){
        this.bucket = bucket1;
        this.codec = codec1;
    }
    toEncodedEntry(e) {
    ...
    put(k, data, opts = {
        ...
        const buf = data ? this.codec.encode(data) : Empty1;
...
export { ErrorCode1 as ErrorCode, isNatsError1 as isNatsError, NatsError1 as NatsError };
...
export { checkJsError1 as checkJsError, isFlowControlMsg1 as isFlowControlMsg, isHeartbeatMsg1 as isHeartbeatMsg, millis1 as millis, nanos1 as nanos };
...
function validateBucket(name1) {
    if (!validBucketRe.test(name1)) {
        throw new Error(`invalid bucket name: ${name1}`);
    }
}
class Bucket1 {
    jsm;
    ...
    bucket;
    codec;
    constructor(bucket2, jsm2, js3){
        validateBucket(bucket2);
        this.jsm = jsm2;
        this.js = js3;
        this.bucket = bucket2;
    }
    static async create(nc, name, opts = {
        ...
        const jsm3 = await nc.jetstreamManager({
            timeout: to
        });
        const bucket3 = new Bucket1(name, jsm3, nc.jetstream({
            timeout: to
        }));
        await bucket3.init(opts);
        return bucket3;
    }
    async init(opts = {
    }) {
        const bo = Object.assign(defaultBucketOpts1(), opts);
        this.codec = bo.codec;
        const sc = {
        };
    ...
    consumerOn(k, lastOnly = false) {
        ...
        const ji = this.js;
        const nc8 = ji.nc;
        const inbox = createInbox1(nc8.options.inboxPrefix);
        const opts15 = {
            "deliver_subject": inbox,
            "deliver_policy": lastOnly ? DeliverPolicy1.LastPerSubject : DeliverPolicy1.All,
            "ack_policy": AckPolicy1.Explicit,
            "filter_subject": this.subjectForKey(ek),
            "flow_control": k === "*"
        };
        return this.jsm.consumers.add(this.stream, opts15);
    }
    async history(k) {
        const ci = await this.consumerOn(k);
        const max2 = ci.num_pending;
        const qi = new QueuedIteratorImpl();
        if (max2 === 0) {
            qi.stop();
            return qi;
        ...
        const sub = nc8.subscribe(subj, {
            callback: (err, msg4)=>{
                ...
                if (err) {
                    ...
                    if (isFlowControlMsg1(msg4) || isHeartbeatMsg1(msg4)) {
                        msg4.respond();
                        return;
                    }
                    qi.received++;
    ...
    async keys() {
        ...
        await (async ()=>{
            for await (const m of sub){
                const err = checkJsError1(m);
                if (err) {
                    sub.unsubscribe();
                    d.reject(err);
                } else if (isFlowControlMsg1(m)) {
                    m.respond();
                } else {
...
export { Bucket1 as Bucket, defaultBucketOpts1 as defaultBucketOpts };
export { EncodedBucket1 as EncodedBucket };
